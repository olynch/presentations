...
export function bibliography(p: SlideParams) {
    ...
    return p.citeproc.makeBibliography()[1].join("")
}
...
export function Env({ taxon, children }: { taxon: string, children?: VNode[] }) {
    return <div class="env"><span class="taxon">{titlecase(taxon)}. </span>
        {children}
    </div>
}
...
export function ocaml(p: SlideParams, s: string) {
    const code = p.highlighter.codeToHtml(s, { lang: 'ocaml', theme: 'github-light' })
...
function createTexMacros(macros: Record<string, string>) {
    return Object.entries(macros).map((m) => String.raw`\newcommand${m[0]}${nargs(m[1])}{${m[1]}}`).join('\n') + String.raw`
...
export function m(strings: TemplateStringsArray) {
    ...
    return <span class="katex"><Raw unsafe={html}/></span>
}
...
export function M(strings: TemplateStringsArray) {
    ...
    return <div class="equation"><span class="katex"><Raw unsafe={html}/></span></div>
}
...
function citeSingle(p: SlideParams, id: string) {
    ...
    const citation = p.citeproc.makeBibliography({ select: [ { field: "id", value: id } ] })[1][0]
    ...
    return <span class="tooltip">{text}<span class="tooltiptext"><Raw unsafe={citationText} /></span></span>
}
...
export function Base({ children, params, title }: { children?: VNode[], params: SlideParams, title?: boolean }) {
...
type Slide = (p: SlideParams) => VNode
...
export async function renderSlides(dir: string, pres: Presentation) {
    ...
    const citeproc = new CSL.Engine({
        retrieveLocale: (_tag: string) => Deno.readTextFileSync('src/en-US.xml'),
    ...
    }, Deno.readTextFileSync("src/acm-siggraph.csl"));
    const ocaml = JSON.parse(await Deno.readTextFile('src/ocaml.json'))
    ...
    for (const source of toBuild) {
        ...
        if (!await fileExists(`_built/${hash}.svg`)) {
            ...
            await (new Deno.Command('dvisvgm', {
                args: ["--exact-bbox", "--clipjoin", "--font-format=woff", "--bbox=papersize", "--zoom=2.3", 'img.dvi'],
                cwd: tmpdir
            })).spawn().status
